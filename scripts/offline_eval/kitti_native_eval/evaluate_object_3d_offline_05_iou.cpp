...
const double MIN_OVERLAP[3][4] = {{0.7, 0.5, 0.5, 0.5}, {0.5, 0.25, 0.25, 0.25}, {0.5, 0.25, 0.25, 0.25}};
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_05_iou.txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_orientation_05_iou.txt").c_str(),"w");
